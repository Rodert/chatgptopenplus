...
export function FloatingSupport() {
  ...
  return (
    <div className="fixed z-50" style={{ left: position.x, top: position.y }}>
      {open ? <div className="absolute bottom-14 right-0 w-64 rounded-lg border border-line bg-white p-4 shadow-[0_16px_36px_rgba(21,70,51,0.18)]"><p className="text-sm font-semibold text-ink">{label}</p><p className="mt-1 text-xs text-muted">{site.support.hours}</p><a href={`mailto:${site.support.email}?subject=${encodeURIComponent(subject)}`} className="mt-4 block rounded-lg border border-brand/25 bg-mint px-3 py-2.5 text-center text-sm font-semibold text-brand transition hover:bg-brand hover:text-white">{emailLabel}</a><p className="mt-3 break-all text-center text-xs text-muted">{site.support.email}</p></div> : null}
